...

{
  programs.zsh.shellAliases = {
    # ── NixOS Rebuild ─────────────────────────────────────────────────
    rebuild = "git -C ${repoDir} fetch origin main && git -C ${repoDir} rebase origin/main && ${repoDir}/scripts/validate-hardware.sh && sudo nixos-rebuild switch --flake ${repoDir}#prague";
