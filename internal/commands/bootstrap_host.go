...
func runHostBootstrap(a *app.App, hostName, userName, repoPath string, tags []string, tailnetDomain string, gitlabRunnerTags []string, sshKeys []string, features []string, force bool) error {
	...
	// Create host spec file (hosts/<name>.nix)
	fmt.Println("  " + t.Dim("Creating host spec file..."))
	if err := createHostSpec(a.Repo.Root, spec, force); err != nil {
		return fmt.Errorf("create host spec: %w", err)
	}
	fmt.Println("  " + t.Good("✓ Host spec created: hosts/"+hostName+".nix"))
	fmt.Println()
...
func createHostSpec(root string, spec template.HostSpec, force bool) error {
	hostSpecPath := filepath.Join(root, "hosts", spec.HostName+".nix")

	// Check if host spec already exists
	if !force {
		if _, err := os.Stat(hostSpecPath); err == nil {
			return fmt.Errorf("host spec %s already exists (use --force to overwrite)", hostSpecPath)
		}
	}

	content := generateHostSpec(spec)
	return os.WriteFile(hostSpecPath, []byte(content), 0644)
}

func generateHostSpec(spec template.HostSpec) string {
	...
	b.WriteString("# -------\n")
	b.WriteString("# Host spec for " + spec.HostName + ". Auto-discovered by hosts/hosts.nix aggregator.\n")
