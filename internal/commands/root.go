...
func rootHelp(a *app.App) string {
	...
	groups := []struct {
		...
		},
		{
			...
			Commands: []string{
				"flow start      Start new workflow (issue + branch)",
				"flow validate   Run all verification gates locally",
				"flow commit     Commit staged changes",
				"flow push       Push branch to GitLab",
				"flow mr         Create merge request",
				"flow pipeline   Monitor CI pipeline status",
				"flow merge      Merge MR (gates on CI)",
				"flow deploy     Deploy after merge",
				"flow rollback   Roll back to previous generation",
				"flow quick      Single-shot: commit → push → MR",
